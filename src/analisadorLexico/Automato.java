...
public class Automato {
	...
	public Automato() {
		...
		this.pilhaAuto = new Stack<Automato>();
	...
	public void construirAuto(Automato a1, Automato a2) {
		...
		for (int i = 0; i < a1.transicoes.size(); i++) {
			this.transicoes.add(new Transicao(this.inicial, a1.transicoes.get(i).getDestino(),
					a1.getTransicoes().get(i).getSimbolo()));
		...
		for (int i = 0; i < a2.transicoes.size(); i++) {
			this.transicoes.add(new Transicao(this.inicial, a2.transicoes.get(i).getDestino(),
					a2.getTransicoes().get(i).getSimbolo()));
	...
	public void concatenaAuto(Automato a1, Automato a2) {
		for (int i = 0; i < a1.transicoes.size(); i++) {
			for (int j = 0; j < a2.transicoes.size(); j++) {
				...
				this.transicoes.add(new Transicao(this.inicial, a2.transicoes.get(j).getOrigem(),
						a1.getTransicoes().get(i).getSimbolo()));
				this.transicoes.add(new Transicao(a2.transicoes.get(j).getOrigem(), a2.transicoes.get(j).getDestino(),
						a2.getTransicoes().get(j).getSimbolo()));
	...
	public void fechoKleen(Automato a3) {
		...
		if (a3.getFinais().size() > 1) {
			for (int i = 0; i < a3.getFinais().size(); i++) {
				a3.transicoes.add(new Transicao(a3.transicoes.get(i).getDestino(), a3.transicoes.get(i).getOrigem(),
						a3.transicoes.get(i).getSimbolo()));
			}

		} else // Para operação .
		{
			for (int i = 0; i < a3.getFinais().size(); i++) {
				a3.transicoes.add(new Transicao(a3.getFinais().get(i), a3.inicial, 'n'));// Repassei n como se não
																							// consumisse nada, podemos
																							// trocar isso
			}
		}
